...
class HDF5Reader:
    ...
    def next_batch(self,batchSize):
        ...
        X = self.curr_data[0:cols-1,self.curr_index:self.curr_index+batchSize]
        #Y = self.curr_label[self.curr_index:self.curr_index+batchSize]
        Y = self.get_labels(self.curr_data[cols-1, self.curr_index:self.curr_index+batchSize],NUM_LABELS)
        ...
        if X.shape[1] == batchSize:
            ...
            X = numpy.vstack([X,self.curr_data[0:cols-1, 0:samples_missing]])
            Y = numpy.vstack([Y,self.get_labels(self.curr_data[cols-1, 0:samples_missing])])
...
def main():
    ...
    X,Y,X_val,Y_val = reader.next_batch(BATCH_SIZE)
